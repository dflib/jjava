...
public class GlobFinderTest {
    ...
    public void test(Configuration fsConfig, String glob, Set<String> files, Set<String> paths) throws Exception {
        ...
        actual = StreamSupport.stream(finder.computeMatchingFiles().spliterator(), false)
                .map(Path::normalize)
                .map(Path::toAbsolutePath)
                .collect(Collectors.toSet());
        ...
        expected = paths.stream()
                ...
                .map(Path::toAbsolutePath)
                .collect(Collectors.toSet());
        actual = StreamSupport.stream(finder.computeMatchingPaths().spliterator(), false)
                .map(Path::normalize)
                .map(Path::toAbsolutePath)
                .collect(Collectors.toSet());
    ...
    public static Stream<Arguments> data() {
        return Stream.of(
                ...
                Arguments.of(
                        WIN_FS,
                        "C:/*/",
                        allDirsFlatMapped(d -> allFilesMapped(s -> "C:/" + d + "/" + s)),
                        allFilesAndDirsMapped(s -> "C:/" + s)
                ),
                Arguments.of(
                        UNIX_FS,
                        "/*/",
                        allDirsFlatMapped(d -> allFilesMapped(s -> "/" + d + "/" + s)),
                        allFilesAndDirsMapped(s -> "/" + s)),
                Arguments.of(
                        OSX_FS,
                        "/*/",
                ...
                Arguments.of(
                        WIN_FS,
                        "C:/dir-?/",
                        allDirsFlatMapped(d -> !d.startsWith("dir-")
                                ? Collections.emptySet()
                                : allFilesMapped(f -> "C:/" + d + "/" + f)),
                ...
                Arguments.of(
                        WIN_FS,
                        "C:/*c*",
                        allFilesFilterMapped(f -> f.contains("c"), f -> "C:/" + f),
                        allFilesAndDirsFilterMapped(f -> f.contains("c"), f -> "C:/" + f)
                ),
                Arguments.of(
                        UNIX_FS,
                        "/*c*",
                        allFilesFilterMapped(f -> f.contains("c"), f -> "/" + f),
